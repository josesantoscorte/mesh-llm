...
| Qwen3 | 0.6B instruct | `Qwen/Qwen3-0.6B-GGUF/Qwen3-0.6B-Q8_0.gguf` | `mlx-community/Qwen3-0.6B-4bit` | `MEDIUM` | 2026-04-04 | `FAIL` | `PASS` | `PENDING` | `FAIL` | `MLX BETTER` | `PARTIAL` | Rebuilt-engine exact rerun confirmed the earlier result. Same family and size, but GGUF uses `Q8_0` while MLX is `4bit`, so parity is informative but not perfect. GGUF again failed the first exact prompt by emitting reasoning prose instead of `blue`, while MLX exact smoke again passed `blue / green / red` and the reasoning-on probe. Sampled MLX behavior still failed badly across writing, roleplay, and reasoning. |
| Qwen3 / JOSIE | 0.6B instruct | `mradermacher/JOSIE-IT1-Qwen3-0.6B-GGUF/JOSIE-IT1-Qwen3-0.6B.Q4_K_M.gguf` | `mlx-community/JOSIE-IT1-Qwen3-0.6B-4bit` | `HIGH` | 2026-04-04 | `FAIL` | `FAIL` | `PENDING` | `PENDING` | `MATCH` | `PARTIAL` | Corrected rebuilt-engine rerun used `chat_template.jinja` on MLX. GGUF still failed the first exact prompt by refusing to answer exactly `blue`, while MLX passed `blue` and `green` but returned an empty assistant message for exact `red`. Both backends fail the strict deterministic suite, so this is still shared weakness rather than a clear MLX-specific regression. |
