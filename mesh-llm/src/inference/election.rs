...
fn resolve_runtime_moe_config(
    ...
    let (ranking, ranking_source, ranking_origin) = match options.ranking_strategy {
        moe::MoeRankingStrategy::Auto => {
                ...
                provider::best_shared_moe_ranking_artifact_for_model(model_path, None)
            {
                ...
                if should_attempt_local_micro_analyze(model_path, model_name, local_vram_budget) {
                    match provider::ensure_micro_analyze_ranking_for_model(
                        bin_dir, model_name, model_path, options, None,
                    ) {
                        Ok(artifact) => (
                            artifact.ranking,
                            artifact.kind.label().to_string(),
        ...
        moe::MoeRankingStrategy::Analyze => {
            ...
            let artifact = provider::ensure_full_analyze_ranking_for_model(
                bin_dir, model_name, model_path, &cached, None,
            )?;
            (
                artifact.ranking,
                artifact.kind.label().to_string(),
        ...
        moe::MoeRankingStrategy::MicroAnalyze => {
            let artifact = provider::ensure_micro_analyze_ranking_for_model(
                bin_dir, model_name, model_path, options, None,
            )?;
            (
                artifact.ranking,
...
pub async fn election_loop(
    ...
    loop {
        ...
        if i_am_host {
            ...
            let (llama_port, process) = match provider::start_distributed_host(
                &node,
                &tunnel_mgr,
                &bin_dir,
                &model,
