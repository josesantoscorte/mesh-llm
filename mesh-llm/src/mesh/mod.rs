    ...
    async fn refresh_peer_connection(
...

        {
            ...
            if let Some(current) = state.connections.get(&peer_id) {
                ...
                tracing::debug!(
                    "Relay health refresh for {} lost the tracked connection slot",
                    peer_id.fmt_short()
                );
                drop(state);
                new_conn.close(0u32.into(), b"relay-health-missing-slot");
                return false;
            }
        }
...

        {
            ...
            if !should_remove_connection(
                ...
                tracing::debug!(
                    "Relay health refresh for {} completed after another connection replaced it",
                    peer_id.fmt_short()
                );
                drop(state);
                new_conn.close(0u32.into(), b"relay-health-raced");
