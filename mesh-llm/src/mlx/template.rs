...
impl PromptTemplate {
    ...
    pub fn behavior(&self) -> crate::models::ModelPromptBehavior {
        match self {
            ...
            PromptTemplate::Olmo2 => crate::models::ModelPromptBehavior {
                prompt_template: Some("olmo2".to_string()),
...
fn render_olmo2(messages: &[Value]) -> String {
    ...
    for (index, message) in messages.iter().enumerate() {
        ...
        prompt.push_str(&message_content_text(message));
        prompt.push('\n');
        if role == "assistant" && index + 1 != messages.len() {
