...
#[test]
fn olmo2_heuristic_fallback_is_selected_for_olmo2_configs() {
    let config = json!({
        "model_type": "olmo2",
        "architectures": ["Olmo2ForCausalLM"]
    });
    assert_eq!(heuristic_prompt_template(&config), PromptTemplate::Olmo2);
}
...
fn render_olmo2_matches_origin_role_marker_shape() {
    ...
    let prompt = render_olmo2(&messages);
    assert!(prompt.starts_with("<|endoftext|><|system|>\nBe concise.\n"));
